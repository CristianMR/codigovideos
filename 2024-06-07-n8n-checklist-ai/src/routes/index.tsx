...
Persona: "Y listo… una pizza deliciosa en minutos. ¡Prueba y disfruta! No olvides seguirme para más recetas rápidas."`);
  ...
  const someItemNotDone = useComputed$(() => Object.values(result.value).some((value) => !value));
  ...
  const handleImproveAI = $(async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5678/webhook-test/checklist-ai-improve-api`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          text: text.value,
          questions: questions.value.filter((question, index) => !result.value[index]),
        }),
      });
      ...
      const data = await response.json() as [{ output: { 'improved-text': string } }];

      text.value = data[0].output['improved-text'];
    } catch(error) {
      alert(error);
    }
    ...
    <>
      <div class="container mx-auto max-w-md">
        ...
        <div class="flex">
          ...
          {someItemNotDone.value &&
          <button class="btn btn-secondary ml-2" onClick$={handleImproveAI} disabled={inProgress.value}>
            {inProgress.value ? 'Mejorando...' : 'Mejorar'}
          </button>}
        </div>
      </div>
    </>
